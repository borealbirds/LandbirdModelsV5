...
get_data_bcr <- function(k, bcr.j, spp.i, b.list) {
  ...
  dat.k <- cbind(visit.k, year.k, bird.k, off.k) |>
    rename(count = bird.k, offset = off.k, year1 = year) |>
    mutate(
  ...
  cov.k$predraw <- suppressWarnings(predict.gbm(
    b.list[[k]],
    cov.k,
    type = "link"
  ))
...
evaluate_boot <- function(data) {
  ...
  #2. Get the AUCs ----
  AUC_init <- simple_auc(simple_roc(
    ifelse(data$test$count > 0, 1, 0),
    data$test$predinit
  ))
  AUC_final <- simple_auc(simple_roc(
    ifelse(data$test$count > 0, 1, 0),
    data$test$predfull
  ))
...
brt_evaluate <- function(i) {
  ...
  for (j in 1:nrow(loop.i)) {
    ...
    #6. Get the models ----
    bload <- try(load(file.path(
      root,
      "output",
      "06_bootstraps",
      spp.i,
      paste0(spp.i, "_", bcr.j, ".Rdata")
    )))
    ...
    #7. Get the data -----
    dat[[j]] <- lapply(seq_along(b.list), function(k) {
      get_data_bcr(k, bcr.j = bcr.j, spp.i = spp.i, b.list = b.list)
    })
    ...
    preds[[j]] <- do.call(
      cbind,
      lapply(dat[[j]], function(b) {
        as.numeric(b$occc$predfull)
      })
    )
  ...
  for (j in 1:nrow(mosaic.i)) {
    ...
    if (bcr.j == "Canada") {
      bcrs.j <- colnames(bcrlist)[str_sub(colnames(bcrlist), 1, 3) == "can"]
    }
    if (bcr.j == "Alaska") {
      bcrs.j <- c("usa41423", "usa2", "usa40", "usa43", "usa5")
    }
    if (bcr.j == "Lower48") {
      bcrs.j <- c(
        "usa5",
    ...
    #15. Evaluate ----
    eval[[nrow(loop.i) + j]] <- do.call(
      rbind,
      lapply(dat[[nrow(loop.i) + j]], evaluate_boot)
    )

    #16. Calculate OCCC -----
    #Need to get a dataframe of predictions with a column for each bootstrap
    preds[[nrow(loop.i) + j]] <- do.call(
      cbind,
      lapply(dat[[nrow(loop.i) + j]], function(b) {
        as.numeric(b$occc$predfull)
      })
    )
  ...
  save(
    eval,
    oc,
    file = file.path(root, "output", "12_validation", paste0(spp.i, ".Rdata"))
  )
}
...
packaged <- data.frame(
  path = list.files(
    file.path(root, "output", "10_packaged"),
    pattern = "*.tif",
    full.names = TRUE,
    recursive = TRUE
  ),
  ...
  separate(
    file,
    into = c("spp1", "bcr1", "spp", "bcr", "year", "filetype"),
    remove = FALSE
  ) |>
  mutate(year = as.numeric(year)) |>
  dplyr::filter(spp1 != "Sampling")
...
todo <- data.frame(
  spp = list.files(file.path(root, "output", "07_predictions"))
)

#3. Check which have been run----
done <- data.frame(
  file.mean = list.files(
    file.path(root, "output", "12_validation"),
    pattern = "*.Rdata"
  )
) |>
...
mosaiced <- data.frame(
  ...
  separate(
    file,
    into = c("region", "sppfolder", "spp", "year", "filetype"),
    remove = FALSE
  ) |>
  mutate(year = as.numeric(year)) |>
...
print("* Evaluating *")
packaged <- parLapply(cl, X = 1:nrow(loop), fun = brt_evaluate)
