...
# current coding for model elements (line 86-103) isn't robust to different ID order in source files, could lead to survey-covariate mismatch - suggest modifying it to left_joins() by id for dat.i and the use of match() in offsets (off.i <- offsets[match(dat.i$id, offsets$id), spp.i])(A.D.)
# Line 110: Gemini suggests "gbm::gbm does not reliably parse variable exclusions (-) when combined with the dot (.) shorthand", suggesting exclusions might be included. I've not seen issues (ID isn't showing up in the predictor sets) but it recommends writing predictors out explicitly (A.D.).
