...
#TO DO FOR V6: Consider the most efficient place to reduce the models by filtering out covs < 0.01 relative influence; current coding for model elements (line 86-103) isn't robust to different ID order in source files, could lead to survey-covariate mismatch - suggest modifying it to left_joins() by id for dat.i and the use of match() in offsets (off.i <- offsets[match(dat.i$id, offsets$id), spp.i])
